...
Blockly.Field.prototype.setText = function(newText) {
  ...
  this.fieldHasChanged(this, this.text_, newText);
...
Blockly.Field.prototype.getAbsoluteXY_ = function() {
  ...
  * @param {!Field} sender The field.
  * @param {Object} oldValue The old field value
  * @param {Object} newValue The new field value
  * @protected
  */
 Blockly.Field.prototype.fieldHasChanged = function(sender, oldValue, newValue) {
   if (sender.sourceBlock_) {
     ...
     var noGroup = false;
     if (sender.sourceBlock_.workspace.options.shadowMorphEnabled &&
         Blockly.Events.recordUndo) {
       if (Blockly.Events.isEnabled()) {
         // In case we are not already in a group of events, we create a new one,
         // so that the potential Change 'shadow' event generated by setShadow()
         // is in the same group of the Change 'field' event.
         noGroup = !Blockly.Events.getGroup();
         if (noGroup) {
           Blockly.Events.setGroup(true);
         }
       }
       sender.sourceBlock_.setShadow(false);
     }
     if (sender.sourceBlock_ && Blockly.Events.isEnabled()) {
       Blockly.Events.fire(new Blockly.Events.Change(
           sender.sourceBlock_, 'field', sender.name, oldValue, newValue));
       if (noGroup) {
